/**
 * price.js — 物价影响页面科学可视化版
 *
 * 经济学框架：
 * 1. Event study：以 2026 年 2 月为基期，比较战争前后价格曲线。
 * 2. Pass-through：展示上游冲击到下游账单的传导速度和口径。
 * 3. Cost share：用成本占比和传导率解释账单测算。
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderBaseline() {
    const container = document.getElementById('chart-chain');
    if (!container) return;
  ...
  function computeReaction(series) {
  ...
  function renderEventStudy() {
    const container = document.getElementById('chart-pass-through');
    if (!container) return;
    ...
    container.innerHTML = `
      <div class="event-study">
        ...
        <div class="small-multiples">
          ${groups.map(group => buildGroupPanel(group)).join('')}
        </div>
  ...
  function renderLagAndTransmission() {
    const container = document.getElementById('chart-global-context');
    if (!container) return;
  ...
  function renderGlobalMatrix() {
    const container = document.getElementById('chart-food-price');
    if (!container) return;
  ...
  function renderInteractiveReceipts() {
    const container = document.getElementById('chart-fertilizer');
    if (!container) return;
    ...
    container.innerHTML = `
      <div class="interactive-model">
        <div class="science-card control-panel">
          ...
          <div class="shock-readout" id="shock-readout" aria-live="polite"></div>
        </div>
        <div class="receipt-grid" id="receipt-output"></div>
      </div>
    ...
    container.querySelectorAll('input[type="range"]').forEach(input => {
      input.addEventListener('input', updateInteractiveReceipts);
    });
    updateInteractiveReceipts();
  }

  function renderDataCautions() {
    const container = document.getElementById('chart-household-receipt');
    if (!container) return;
  ...
  function updateInteractiveReceipts() {
    const output = document.getElementById('receipt-output');
    if (!output) return;

    const oilPrice = Number(document.getElementById('oilPrice')?.value || 118);
    const commuteMiles = Number(document.getElementById('commuteMiles')?.value || 660);
    const mpg = Number(document.getElementById('mpg')?.value || 25);
    const fertilizerShock = Number(document.getElementById('fertilizerShock')?.value || 80);

    document.querySelectorAll('.control-row input').forEach(input => {
      const out = input.closest('.control-row')?.querySelector('output');
      if (out) out.textContent = `${input.value}${input.dataset.unit}`;
    });

    const oilShock = (oilPrice - model.formulas.usBaseOil) / model.formulas.usBaseOil;
    const usGasAfter = model.formulas.usBaseGas * (1 + model.formulas.oilCostShare * oilShock);
    const usOldBill = (commuteMiles / mpg) * model.formulas.usBaseGas;
    const usNewBill = (commuteMiles / mpg) * usGasAfter;
    const chinaNewTank = model.formulas.chinaBaseTank * (1 + model.formulas.chinaPassThrough * oilShock);
    const indiaFiscal = model.formulas.indiaFiscalShock * (fertilizerShock / 80);

    const readout = document.getElementById('shock-readout');
    if (readout) {
      readout.innerHTML = `
        <span>你设定的油价冲击 <b>${oilShock >= 0 ? '+' : ''}${Math.round(oilShock * 100)}%</b></span>
        <span>→ 美国汽油 <b>$${model.formulas.usBaseGas.toFixed(2)} → $${usGasAfter.toFixed(2)}/gal</b></span>
    ...
    const cards = [
      {
        ...
        note: `公式：${commuteMiles} mi ÷ ${mpg} mpg × $${usGasAfter.toFixed(2)}/gal。汽油价格 = $${model.formulas.usBaseGas.toFixed(2)} ×〔1 + 57% × ${Math.round(oilShock * 100)}%〕。`,
        color: amber,
      },
      {
        ...
        before: `${formatMoney(model.formulas.chinaBaseTank)} 元`,
        after: `${formatMoney(chinaNewTank)} 元`,
        delta: `+${formatMoney(chinaNewTank - model.formulas.chinaBaseTank)} 元/箱`,
        note: `公式：${formatMoney(model.formulas.chinaBaseTank)} ×〔1 + 17% × ${Math.round(oilShock * 100)}%〕。传导率低，反映国内成品油调价机制缓冲。`,
        color: red,
      },
      {
        place: 'India',
